...
const rangeInMs = 5000
describe('Vehicle data buffer', () => {
    it("adds first item without flushing", () => {
        const buffer = new VehicleDataBuffer(rangeInMs)
    ...
    it("adds close enough items without flushing", () => {
        const buffer = new VehicleDataBuffer(rangeInMs)
        const date = Math.floor(new Date().getTime() / rangeInMs) * rangeInMs //A value aligned with a 5-second tick
